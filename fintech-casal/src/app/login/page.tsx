...
export default function LoginPage() {
  ...
  return (
    <div style={{ minHeight: '100dvh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: 'var(--bg-primary)', padding: 24, position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ width: '100%', maxWidth: 360, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 28, position: 'relative', zIndex: 1 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
          <div style={{
            ...
            boxShadow: '0 8px 32px rgba(200,80,192,0.4)',
            border: '2px solid rgba(255,255,255,0.1)',
          }}>
            <img src="/icons/icon-192x192.png" alt="Logo" style={{ width: '100%', height: '100%', objectFit: 'cover' }} onError={e => { (e.currentTarget as HTMLImageElement).style.display = 'none'; }} />
            <span style={{ fontSize: 32, display: 'none' }}>💑</span>
          </div>
          <div style={{ textAlign: 'center' }}>
            <h1 style={{ fontSize: 26, fontWeight: 800, color: 'var(--text-primary)', margin: 0, letterSpacing: '-0.3px' }}>CasalFinance</h1>
            <p style={{ fontSize: 13, color: 'var(--text-muted)', margin: '4px 0 0' }}>Finanças a dois, juntos 💑</p>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', width: '100%', gap: 12 }}>
          ...
          <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Ou</span>
          <div style={{ flex: 1, height: 1, background: 'var(--border-color)' }} />
        </div>
        ...
        <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: 10 }}>
          <button
            ...
            style={{
              ...
              padding: '14px', background: 'var(--bg-card)', border: '1.5px solid var(--border-color)',
              borderRadius: 14, fontWeight: 700, fontSize: 14, color: 'var(--text-primary)',
              cursor: 'pointer', transition: 'all 0.2s', opacity: isLoading ? 0.6 : 1,
            }}
          >
          ...
          {isPwaActive && pinHash && (
            <button onClick={() => setShowPinScreen(true)} type="button" disabled={isLoading} style={{
              width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10,
              padding: '14px', background: 'rgba(167,139,250,0.08)', border: '1.5px solid rgba(167,139,250,0.2)',
              borderRadius: 14, fontWeight: 700, fontSize: 14, color: '#a78bfa',
              cursor: 'pointer', transition: 'all 0.2s', opacity: isLoading ? 0.6 : 1,
            }}>
              <Key size={18} />
              Entrar com PIN
        ...
        <p style={{ fontSize: 13, color: 'var(--text-muted)', textAlign: 'center' }}>
          Ainda não tem conta?{' '}
          <Link href="/signup" style={{ color: '#C850C0', fontWeight: 700, textDecoration: 'none' }}>Cadastre-se</Link>
        </p>
